...
Both complete request bodies have a 32 MiB limit. The actual submission body is checked after the server supplies its opaque transaction ID and auth key, before sending the second request. A batch can pass `Prepare` and exceed this second bound. That failure retains the known transaction ID and reports a definite pre-send failure; the intent already exists, but no submission is attempted. Metadata has no separate arbitrary length limit. The dispatcher applies the same 32 MiB bound to every request body, answering 413 `payload_too_large` (`client.CodePayloadTooLarge`, an ordinary `*HTTPError`) to a declared length above it and cutting a body of unknown length at it; the SDK declares the length and checks the envelope before sending, so its own check is what a caller sees.
...
- `Status` requires a lowercase canonical UUID and returns the reported state, error and executor ID. Unknown state strings are preserved.
...
- `Cancel` sends the dispatcher's cancellation and returns `nil` on the server's acknowledgement (HTTP 204), which says that the cancellation is recorded as the job's result, or that the job already had one. Acknowledgement does not certify remote termination. While the job's control session is live, cancellation of a finished job also requires the executor's acknowledgement; if the executor no longer holds it, cancellation is refused. When the stored control session has ended, the dispatcher records cancellation locally without contacting an executor, or acknowledges an already-terminal job without changing its result. A newly recorded cancellation then says that the control session had ended and the executor's outcome was not observed. `Status` preserves the server's error verbatim: `cancelled via API` for cancellation through a live session, that reason plus the unobserved-outcome qualification for a dead session, or the earlier exit error for an already-terminal job. A 500 `internal_error` saying the cancellation was acknowledged but its result was not recorded means the dispatcher could not record that result, or could not confirm that it did: the executor may be stopping the job even if its recorded state does not show the cancellation, so read `Status` before deciding, and repeat `Cancel` to record it. Not-found, refused and other server-reported cancellation errors are `*HTTPError` values. A transport failure or deadline after `Cancel` sent the request leaves the outcome unknown: `client.IsTransportError` identifies it, and the caller reads `Status` and repeats `Cancel`.
