...
- `Cancel` sends the dispatcher's cancellation and returns `nil` on the server's acknowledgement (HTTP 204). Acknowledgement does not certify remote termination or durable terminal reporting. The current server also acknowledges cancellation of a job that has already finished, and it never rewrites a recorded terminal result: `Status` afterwards reports the server's own error (`cancelled via API` for a job cancelled while nonterminal, or the earlier exit error), which the client preserves verbatim. Not-found, refused and other server-reported cancellation errors are `*HTTPError` values.
