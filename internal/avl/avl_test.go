...
func TestFind(t *testing.T) {
	...
	if x := tree.Get("SOLUTION"); x == nil || x.ID != "SOLUTION" {
...
// Rebalancing can move a node into the subtree of another node holding the
// same value, so identity, not value, decides which member a deletion removes.
func TestDeleteEqualValues(t *testing.T) {
	for _, count := range []int{2, 3, 8, 33, 129} {
		t.Run(fmt.Sprint(count), func(t *testing.T) {
			tree := AVL[int]{}
			for id := range count {
				tree.Insert(id, 1000)
			}
			if tree.Len() != int64(count) || tree.Sum() != int64(count)*1000 {
				t.Fatalf("insert: len=%d sum=%d", tree.Len(), tree.Sum())
			}
			for removed := range count {
				tree.Delete(removed)
				if got := tree.Len(); got != int64(count-removed-1) {
					t.Fatalf("after deleting %d: len=%d want=%d", removed, got, count-removed-1)
				}
				if got := tree.Sum(); got != int64(count-removed-1)*1000 {
					t.Fatalf("after deleting %d: sum=%d", removed, got)
				}
				if tree.Get(removed) != nil {
					t.Fatalf("deleted member %d is still reachable", removed)
				}
				for remaining := removed + 1; remaining < count; remaining++ {
					if node := tree.Get(remaining); node == nil || node.ID != remaining {
						t.Fatalf("deleting %d lost member %d", removed, remaining)
					}
				}
			}
			if tree.Len() != 0 || tree.Sum() != 0 || tree.Fairshare(5000) != 0 {
				t.Fatalf("emptied tree retains members: len=%d sum=%d", tree.Len(), tree.Sum())
			}
		})
	}
}
