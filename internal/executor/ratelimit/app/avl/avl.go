...
func (toDelete *Node[T]) deleteNode(refs map[T]*Node[T]) *Node[T] {
	...
	toDelete.R = toDelete.R.delete(refs, toReplace.ID, toReplace.Value)

	toDelete.recompute()
	return toDelete.rotate()
}

func (n *Node[T]) delete(refs map[T]*Node[T], id T, v int64) *Node[T] {
	if n == nil {
		return nil
	}
	if v == n.Value && id == n.ID {
		return n.deleteNode(refs)
	} else if v <= n.Value {
		n.L = n.L.delete(refs, id, v)
	} else if v > n.Value {
		n.R = n.R.delete(refs, id, v)
	}
...
func (a *AVL[T]) Delete(id T) {
	...
	if a.root.Value == old.Value && a.root.ID == id {
		a.root = a.root.deleteNode(a.refs)
	} else {
		a.root = a.root.delete(a.refs, id, old.Value)
	}
	delete(a.refs, id)
}
