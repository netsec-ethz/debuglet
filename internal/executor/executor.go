...
func getClientCredentials(cfg *config.ExecutorConfig) (*tls.Config, credentials.TransportCredentials, error) {
	return loadClientCredentials(cfg, time.Now())
}

func loadClientCredentials(cfg *config.ExecutorConfig, now time.Time) (*tls.Config, credentials.TransportCredentials, error) {
	cert, err := tlsfiles.KeyPair("credentials.client_cert", cfg.Credentials.ClientCert,
		"credentials.client_key", cfg.Credentials.ClientKey, now)
	if err != nil {
		return nil, nil, err
	}
	...
	if cfg.Credentials.CACert != "" {
		roots, err := tlsfiles.TrustRoots("credentials.ca_cert", cfg.Credentials.CACert, now)
		if err != nil {
			return nil, nil, err
		}
