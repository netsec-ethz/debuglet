...
// Package bpf provides a stub eBPF tagger for non-Linux platforms.
//
...
// Callers should fall back to the pure-Go tagger (pkg/tagger).
func NewBPFTagger(ifaceName string, schedule *tesla.KeySchedule, measurementID []byte) (*BPFTagger, error) {
	return nil, fmt.Errorf("bpf: eBPF tagger is only available on Linux (current platform is non-Linux)")
}
