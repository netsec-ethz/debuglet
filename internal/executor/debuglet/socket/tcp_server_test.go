...
func TestTCPServerEnabled(t *testing.T) {
	s, err := NewTCPServer("", "2022")
	if err != nil {
	...
	s, err = NewTCPServer("203.0.113.10", "2022")
	if err != nil {
		t.Fatalf("NewTCPServer: %v", err)
	}
	if !s.Enabled() {
	...
	s, err = NewTCPServer("203.0.113.10", "")
	if err != nil {
		t.Fatalf("NewTCPServer: %v", err)
	}
	if s.Enabled() {
...
func TestTCPServerListenDistinctPorts(t *testing.T) {
	p1, p2 := freePort(t), freePort(t)
	s, err := NewTCPServer("203.0.113.10", strconv.Itoa(p1)+","+strconv.Itoa(p2))
	if err != nil {
		t.Fatalf("NewTCPServer: %v", err)
	}

	lis1, got1, addr1, err := s.Listen()
	if err != nil {
		t.Fatalf("Listen: %v", err)
	}
	defer lis1.Close()
	lis2, got2, _, err := s.Listen()
	if err != nil {
		t.Fatalf("Listen: %v", err)
	}
	defer lis2.Close()
	...
	if got1 == got2 {
		t.Fatalf("Listen returned the same port twice: %d", got1)
	}
	if addr1 != "203.0.113.10:"+strconv.Itoa(got1) {
		t.Fatalf("Listen addr = %q, want %q", addr1, "203.0.113.10:"+strconv.Itoa(got1))
	}
}

func TestTCPServerReleaseReusesPort(t *testing.T) {
	p := freePort(t)
	s, err := NewTCPServer("203.0.113.10", strconv.Itoa(p))
	if err != nil {
		t.Fatalf("NewTCPServer: %v", err)
	}

	lis1, got1, _, err := s.Listen()
	if err != nil {
		t.Fatalf("Listen: %v", err)
	}
	lis1.Close()
...
func TestTCPServerListenExhausted(t *testing.T) {
	p := freePort(t)
	s, err := NewTCPServer("203.0.113.10", strconv.Itoa(p))
	if err != nil {
		t.Fatalf("NewTCPServer: %v", err)
	...
	if _, _, _, err := s.Listen(); err == nil {
		t.Fatal("Listen should error when all ports are taken")
	}
}
