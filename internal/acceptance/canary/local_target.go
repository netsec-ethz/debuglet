...
// canaryTarget accepts one connection. Success means its exact ACK was
...
func startCanaryTarget(ctx context.Context, nonce string) (*canaryTarget, error) {
	var lc net.ListenConfig
	lis, err := lc.Listen(ctx, "tcp", "127.0.0.1:0")
	if err != nil {
		return nil, fmt.Errorf("start demo target: %w", err)
	}
	t := &canaryTarget{lis: lis, done: make(chan struct{})}
	go func() {
...
func (t *canaryTarget) addr() string          { return t.lis.Addr().String() }
func (t *canaryTarget) Done() <-chan struct{} { return t.done }

func (t *canaryTarget) exchange(ctx context.Context, nonce string) error {
	defer t.lis.Close()
...
func (t *canaryTarget) wait(ctx context.Context) error {
	select {
	case <-t.done:
		return t.err
	default:
...
func (t *canaryTarget) stop(ctx context.Context) error {
	t.close()
	select {
	case <-t.done:
